...
const ButtonModal = ({ modalOpen, onClose, ticketId }) => {
  ...
  const handleUploadPIX = async (sendkey_type, sendmerchant_name, sendKey, ticketId) => {
  setLoading(true);

  try {
    if (!sendkey_type || !sendKey) {
      toastError('Selecione o tipo de chave e informe a chave PIX.');
      setLoading(false);
      return;
    }
  ...
  const createMessage = async () => {
    ...
    try {
      if (selectedOption === "Lista") {
        ...
        await handleUploadPIX(sendkey_type, sendmerchant_name, sendKey, ticketId);
      } else {
