...
const normalizeCampaignContactNumber = (value) => {
...
const ContactSchema = Yup.object().shape({
  ...
  number: Yup.string()
    .required("Required")
    .test(
      "valid-campaign-number",
      "Use o formato: código do país + DDD + número (8 ou 9 dígitos). Ex.: 5511999999999",
      value => {
...
const ContactListItemModal = ({
  ...
  const handleSaveContact = async (values) => {
    const payload = {
      ...values,
      number: normalizeCampaignContactNumber(values.number),
    };
