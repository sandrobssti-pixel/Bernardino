...
const Companies = () => {
    ...
    const [selectedCompanyForConnections, setSelectedCompanyForConnections] = useState(null); //
    ...
    const handleOpenConnectionsModal = async (company) => {
        try {
            const { data } = await api.get("/whatsapp/all/?session=0");
            const filtered = data.filter((w) => w.companyId === company.id);
            setCompanyConnections(filtered);
            setSelectedCompanyForConnections(company);
    ...
    const handleCloseConnectionsModal = () => {
        ...
        setSelectedCompanyForConnections(null);
    };
    ...
    return (
        <MainContainer>
            ...
            <WhatsAppModalCompany
                key={selectedCompanyForConnections?.id ?? "new"}
                open={connectionsModalOpen}
                onClose={handleCloseConnectionsModal}
                filteredWhatsapps={companyConnections}
                companyInfos={selectedCompanyForConnections}
            />
            <MainHeader>
                                ...
                                <TableRow style={rowStyle(company)} key={company.id}>
                                    ...
                                    <TableCell align="center">
                                        <IconButton
                                            ...
                                            onClick={() => handleOpenConnectionsModal(company)}
                                        >
                                            <ListIcon />
                                        </IconButton>
