...
const MetricCard = ({ icon, iconBg, title, value, subtitle, trend, trendValue, badge, badgeColor, badgeBg }) => {
  const getTrendIcon = () => {
    ...
    return <Activity sx={{ fontSize: 12, color: '#6b7280' }} />;
  };
  ...
  return (
    ...
    >
      ...
      <Box sx={{ mb: 0.5 }}>
        <Typography sx={{ fontSize: '1.25rem', fontWeight: 700, color: DASHBOARD_TEXT_PRIMARY, lineHeight: 1.2 }}>
          {value}
        </Typography>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', mt: 1 }}>
        ...
        {subtitle && (
          <Typography sx={{ fontSize: '0.75rem', color: DASHBOARD_TEXT_SECONDARY, ml: 0.5 }}>
            {subtitle}
          </Typography>
        )}
      </Box>
...
const ChartContainer = ({ title, subtitle, children, actions, headerProps = {} }) => (
  <Paper
    elevation={0}
    sx={{
      borderRadius: 3,
  ...
  >
    <Box sx={{ 
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <Box>
          <Typography sx={{ fontSize: '1rem', fontWeight: 600, color: DASHBOARD_TEXT_PRIMARY }}>
            {title}
          </Typography>
          <Typography sx={{ fontSize: '0.75rem', color: DASHBOARD_TEXT_SECONDARY }}>
            {subtitle}
