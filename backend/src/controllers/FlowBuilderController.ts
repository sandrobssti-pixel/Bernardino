...
import UpdateFlowBuilderActiveService from "../services/FlowBuilderService/UpdateFlowBuilderActiveService";
import TestHttpRequestNodeService from "../services/FlowBuilderService/TestHttpRequestNodeService";
...
export const updateFlowActive = async (
  req: Request,
  res: Response
): Promise<Response> => {
  const { idFlow } = req.params;
  ...
  const flowId = parseInt(idFlow, 10);
  const activeValue =
    typeof active === "string" ? active.toLowerCase() === "true" : Boolean(active);

  const flow = await UpdateFlowBuilderActiveService({
    companyId,
    flowId,
    active: activeValue
  });
...
export const testHttpRequest = async (
  ...
  const { companyId } = req.user;
  const { nodeData, variables } = req.body;

  const result = await TestHttpRequestNodeService({
    companyId,
    nodeData,
    variables
  });
