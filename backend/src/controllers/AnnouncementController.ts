...
const normalizeDisplayMode = (value?: string): "popover" | "topbar" =>
  value === "topbar" ? "topbar" : "popover";

const normalizeBackgroundColor = (
  displayMode: "popover" | "topbar",
  value?: string
): string | null => {
  if (displayMode !== "topbar") return null;
...
export const store = async (req: Request, res: Response): Promise<Response> => {
  ...
  const displayMode = normalizeDisplayMode(data.displayMode);
  const backgroundColor = normalizeBackgroundColor(displayMode, data.backgroundColor);
