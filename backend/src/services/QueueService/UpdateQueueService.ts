...
const UpdateQueueService = async (
  ...
  const { color, name, chatbots, userIds } = queueData;
...
if (chatbots) {
  await Promise.all(
    chatbots.map(async bot => {
      if (!bot.name && (bot.optQueueId || bot.optUserId || bot.optIntegrationId)) {
        bot.name = `Opção de Redirecionamento`;
      }
  ...
  delete (queuePayload as Partial<QueueData>).chatbots;
  delete (queuePayload as Partial<QueueData>).userIds;

  await queue.update(queuePayload);
