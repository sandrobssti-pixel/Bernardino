...
const ListMessagesService = async ({
  ...
  const { count, rows: messages } = await Message.findAndCountAll({
    ...
    attributes: ["id", "fromMe", "mediaUrl", "body", "mediaType", "ack", "createdAt", "ticketId", "isDeleted", "queueId", "isForwarded", "isEdited", "isPrivate", "companyId", "dataJson"],
    limit,
    include: [
      {
        model: Contact,
    ...
    offset,
    subQuery: false,
    // Ordenação estável para paginação com offset:
    // sem desempate por id, mensagens com mesmo createdAt podem "sumir" entre páginas.
