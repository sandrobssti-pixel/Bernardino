...
const ListTicketsService = async ({
  ...
  const queueScope = showTicketWithoutQueue
    ? { [Op.or]: [queueIds, null] }
    : { [Op.in]: queueIds };
  ...
  includeCondition = [
    ...
    },
    {
      model: Whatsapp,
      as: "whatsapp",
      attributes: ["id", "name", "color", "channel", "provider", "expiresTicket", "groupAsTicket", "promptId", "complationMessage"]
    },
    {
  ...
  if (status === "ia") {
    whereCondition = {
      ...whereCondition,
      [Op.or]: [{ useIntegration: true }, { isBot: true }]
    };
  }

  if (status && status !== "search" && status !== "ia") {
    whereCondition = {
      ...whereCondition,
        ...
        if (searchOnMessages === "true") {
          ...
          whereCondition = {
            ...
            [Op.or]: [
              ...
              where(
                fn("LOWER", fn("unaccent", col("lastMessage"))),
                "LIKE",
                `%${sanitizedSearchParam}%`
              )
            ]
